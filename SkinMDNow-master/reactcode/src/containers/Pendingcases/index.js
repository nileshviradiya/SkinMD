import React, { Component } from 'react';
import { connect } from 'react-redux';
import { Link } from 'react-router-dom';
import notification from '../../components/notification';
// import Box from '../../components/utility/box';
import HelperText from '../../components/utility/helper-text';
import LayoutWrapper from '../../components/utility/layoutWrapper';
import PageHeader from '../../components/utility/pageHeader';
import IntlMessages from '../../components/utility/intlMessages';
import Scrollbars from '../../components/utility/customScrollBar';
import Button from '../../components/uielements/button';
import skincaseActions from '../../redux/skincase/actions';
import CardWrapper, { Box, StatusTag } from './skincase.style';
import TableWrapper from '../Tables/antTables/antTable.style';

class Skincases extends Component {
  state = {
    selected: [],
    casePath: ''
  };
  componentDidMount() {
    const { initialSkincases, initData } = this.props;

    if (!initialSkincases) {
      debugger
      initData();
      debugger
    }
    
    this.setState({casePath: this.props.match.path});
    if(this.props.match.path == '/dashboard/'){
      this.setState({casePath: this.props.match.path + 'skincase'});
    }
    
  }
  columns = [
    {
      title: 'Number',
      dataIndex: 'number',
      rowKey: 'number',
      width: '15%',
      render: text => <span>{text}</span>,
    },
    {
      title: 'Bill From',
      dataIndex: 'billFrom',
      rowKey: 'billFrom',
      width: '25%',
      render: text => <span>{text}</span>,
    },
    {
      title: 'Bill TO',
      dataIndex: 'billTo',
      rowKey: 'billTo',
      width: '22%',
      render: text => <span>{text}</span>,
    },
    {
      title: 'Total Cost',
      dataIndex: 'totalCost',
      rowKey: 'totalCost',
      width: '15%',
      render: text => <span>{text}</span>,
    },
    {
      title: 'Status',
      dataIndex: 'orderStatus',
      rowKey: 'orderStatus',
      width: '13%',
      render: (text, orderStatus) => {
        let className;
        if (text === 'shipped' || text === 'Shipped' || text === 'SHIPPED') {
          className = 'shipped';
        } else if (
          text === 'delivered' ||
          text === 'Delivered' ||
          text === 'DELIVERED'
        ) {
          className = 'delivered';
        } else if (
          text === 'pending' ||
          text === 'Pending' ||
          text === 'PENDING'
        ) {
          className = 'pending';
        }
        return <StatusTag className={className}>{text}</StatusTag>;
      },
    },
    {
      title: '',
      dataIndex: 'view',
      rowKey: 'view',
      width: '10%',
      render: (text, skincase) => (
        <div className="isoInvoiceBtnView">
          <Link to={`${this.state.casePath}/${skincase.id}`}>
            <Button color="primary" className="skincaseViewBtn">
              View
            </Button>
          </Link>{' '}
          <Button
            className="skincaseDltBtn"
            // icon="delete"
            onClick={() => {
              notification('error', '1 skincase deleted');
              this.props.deleteSkincase([skincase.key]);
              this.setState({ selected: [] });
            }}
          >
            <i className="ion-android-delete" />
          </Button>
        </div>
      ),
    },
  ];
  getnewSkincaseId = () => new Date().getTime();
  render() {
    const { match, skincases, deleteSkincase } = this.props;
    const { selected } = this.state;
    const rowSelection = {
      hideDefaultSelections: true,
      selectedRowKeys: selected,
      onChange: selected => this.setState({ selected }),
      selections: [
        {
          key: 'all-data',
          text: 'Select All skincases',
          onSelect: () =>
            this.setState({
              selected: this.props.skincases.map(skincase => skincase.key),
            }),
        },
        {
          key: 'no-data',
          text: 'Unselect all',
          onSelect: () => this.setState({ selected: [] }),
        },
        {
          key: 'delete-selected',
          text: 'Delete selected',
          onSelect: changableRowKeys => {
            if (selected.length > 0) {
              if(window.confirm("Do you want to delete selected records?")){
                deleteSkincase(selected);
                this.setState({ selected: [] });
                notification('error', `${selected.length} skincases deleted`);
              }
            }
          },
        },
      ],
      onSelection: selected => this.setState({ selected }),
    };
    return (
      <LayoutWrapper>
        <PageHeader>
          <IntlMessages id="sidebar.pendingcases" />
        </PageHeader>
        <Box>
          <div className="isoInvoiceTableBtn">
            <h3><span style={{color: '#788195'}}>My Pending Cases - Active Cases</span></h3>
            <Link to={`${match.path}/${this.getnewSkincaseId()}`} style={{display:'none'}}>
              <Button type="primary" className="mateAddInvoiceBtn">
                Add Skincase
              </Button>
            </Link>
          </div>

          <CardWrapper title="Skincases">
            {skincases.length === 0 ? (
              <HelperText text="No Cases" />
            ) : (
              <div className="isoInvoiceTable">
                <Scrollbars style={{ width: '100%' }}>
                  <TableWrapper
                    rowSelection={rowSelection}
                    dataSource={skincases}
                    columns={this.columns}
                    pagination={false}
                    className="invoiceListTable"
                  />
                </Scrollbars>
              </div>
            )}
          </CardWrapper>
        </Box>
      </LayoutWrapper>
    );
  }
}

function mapStateToProps(state) {
  return {
    ...state.Skincases,
  };
}
export default connect(
  mapStateToProps,
  skincaseActions
)(Skincases);
