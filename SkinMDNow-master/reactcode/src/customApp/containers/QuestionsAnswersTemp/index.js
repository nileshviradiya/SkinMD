...
class QuestionAnswers extends Component {
  ...
  render() {
      ...
      },
      {
        ...
        render: (text, qa_list) => (
            <div className="isoInvoiceBtnView">
              <Link to={`${'questions-answer-template'}/${qa_list.key}`}>
                <Button color="primary" className="skincaseViewBtn">
                  View
                </Button>
              </Link>{' '}
              <Popconfirm title="Sure to delete?" onConfirm={() => this.handleDelete(qa_list.key) }>            
                <Button
    ...
    const { qa_list, deleteQA } = this.props;
      ...
      selections: [
        {
          ...
          onSelect: () =>
            this.setState({
              selected: this.props.qa_list.map(qa => qa.key),
            }),
        },
        {
          key: 'no-data',
    ...
    return (
      <LayoutWrapper>
        <PageHeader>
          <IntlMessages id="sidebar.quesanstemp" />
        </PageHeader>
        <Box>   
          <div className="isoInvoiceTableBtn">            
            <Link to={`${'addquestionanswer'}`}>
